...
export default class TerrainHarvestSubCommand extends SubSlashCommand {
    ...
    public name = "harvest";
    public description = "Utilize esse comando para fazer a colheita em um terreno.";
    ...
    public async run(_: Bot, interaction: ChatInputCommandInteraction<"cached" | "raw">) {
        ...
        for (const slot of toHarvestSlots) {
            ...
            const harvestData: HarvestData = harvestProducts.find(p => p.id === seed.seed_id) || {
                id: seed.seed_id,
        ...
        for (const product of harvestProducts) {
           await db.item.findOneAndUpdate({ id: product.product_id, owner: interaction.user.id }, {
               $setOnInsert: {
        ...
        return interaction.editReply({
            content: `${emojis.icons_correct} ${emojis.icons_text5} **Colheita!** ${interaction.user}, você fez a **colheita** no **terreno** \`${terrainId}\` e obteve:\n` +
                harvestBoard
        })
    }
}
